...
const PROMPT_FILE = path.join(process.cwd(), 'scoring-prompt.txt');
...
// Read on every call, like voice-skill.txt, so the prompt can be tuned without code changes.
export async function loadScoringPrompt() {
  let text;
  try {
    text = await readFile(PROMPT_FILE, 'utf8');
  } catch (err) {
    throw new ScoringError(`Could not read ${PROMPT_FILE}: ${err.message}`);
  }
  text = text.replace(/^﻿/, '').trim();
  if (!text.includes(NOTE_PLACEHOLDER)) {
    throw new ScoringError(`scoring-prompt.txt must contain ${NOTE_PLACEHOLDER}`);
  }
  return text;
}
