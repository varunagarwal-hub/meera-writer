...
const TASK_RULES = `
TASK RULES:
- The voice instructions above are the source of truth for how the author thinks, his tone, vocabulary and every other stylistic rule, except where the POST CRAFT RULES override them on length, structure, hooks and format.
...
- Write a LinkedIn post: pick the mode and structure the voice instructions describe, shaped by the POST CRAFT RULES.
- Plain text only. The post is sent as a Telegram message and pasted into LinkedIn, so no markdown: no **bold**, no asterisks or underscores for emphasis.
- Do not invent facts, numbers, dates, anecdotes, studies, quotes or metrics that are not in the raw note, the voice instructions, or a news item supplied below the note. Where a fact is needed but missing, write around it as the voice instructions describe.
...
export async function buildPrompt(note, { newsPrompt } = {}) {
  ...
  const postStyle = await loadPromptFile('post-style.txt');
  return {
    systemInstruction:
